...
test('withUsageNormalized passes raw vendor cache fields through unchanged; rewrite is the vendor interceptors job', async () => {
  // This interceptor used to rewrite DeepSeek prompt_cache_hit_tokens and
  // Kimi cached_tokens into prompt_tokens_details.cached_tokens. That field
  // rewriting now lives on vendor-deepseek-normalize / vendor-kimi-normalize.
  // Here we verify the generic interceptor passes the raw vendor fields
  // through untouched when no vendor flag is set.
  const frames = await runWithFrames(
    eventFrame({
      id: 'x',
      object: 'chat.completion.chunk',
