...
// confirmed its usage object never emits cached fields. The 5m vs 1h cache
// write split is the wire-level shape on Anthropic's side (see MessagesUsage
// `cache_creation`); from this table's perspective each is just another
// disjoint dimension with its own unit price.
...
// An unknown or absent tier returns the base snapshot unchanged (without
// `tiers`).
