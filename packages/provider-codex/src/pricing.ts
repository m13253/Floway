...
// provider. Codex itself bills as a flat-fee subscription rather than
// per-token, but the gateway tracks usage cost as if the operator were paying
// OpenAI's public API rates — that lets the dashboard surface "value consumed
// vs. flat fee" so the operator can see whether a subscription is paying off
// relative to direct API spend. Values are USD per million tokens, aligned
// with the `Cost` schema in models.dev:
// https://github.com/anomalyco/models.dev/blob/8e6d393c01cb42d41a92f18725eef545e7190efb/packages/core/src/schema.ts
//
// Source of truth for OpenAI public API prices: https://openai.com/api/pricing/.
//
// Per-tier overrides cover the two service tiers Codex CLI exposes: `flex`
// (50% off, slower) is set via `service_tier: "flex"`; `priority` (premium
// for guaranteed latency) is what Codex CLI's `/fast` toggle writes, per
// https://github.com/openai/codex/blob/3a2712ea141a5acb243689f803f26e48ff751771/codex-rs/protocol/src/config_types.rs
