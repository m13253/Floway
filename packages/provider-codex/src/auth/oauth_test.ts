...
describe('exchangeCodexAuthorizationCode', () => {
  test('POSTs form data and returns parsed tokens', async () => {
    ...
    const result = await exchangeCodexAuthorizationCode({ code: 'CODE', codeVerifier: 'VER', state: 'STATE', fetcher: directFetcher });
  ...
  test('throws session-terminated on app_session_terminated', async () => {
    ...
    await expect(exchangeCodexAuthorizationCode({ code: 'CODE', codeVerifier: 'VER', state: 'STATE', fetcher: directFetcher })).rejects.toBeInstanceOf(CodexOAuthSessionTerminatedError);
  });
  ...
  test('throws generic error on other 4xx, message includes status', async () => {
    ...
    await expect(exchangeCodexAuthorizationCode({ code: 'CODE', codeVerifier: 'VER', state: 'STATE', fetcher: directFetcher })).rejects.toThrow(/400/);
  });
});
