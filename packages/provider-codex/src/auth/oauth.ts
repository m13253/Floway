...
// sub2api's `isNonRetryableRefreshError`, which shares the same list across
// OpenAI/Claude/Gemini OAuth — Codex is OpenAI OAuth, so the set carries
// over verbatim:
// https://github.com/Wei-Shaw/sub2api/blob/9b270f11d7444550402cbc68bd13530457efe59e/backend/internal/service/token_refresh_service.go#L429
// `invalid_grant` is included even though the refresh-race recovery in
// access-token-cache.ts may re-classify it when a sibling rotation is
// detected; from the OAuth wire's perspective it is still a terminal signal.
