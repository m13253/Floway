-- Widen the upstreams.provider CHECK constraint to include 'ollama'. SQLite
-- cannot alter a CHECK in place; rebuild the table and copy the rows
-- verbatim, following the 0027 rename-and-rebuild pattern.

CREATE TABLE upstreams_new (
  id                         TEXT PRIMARY KEY,
  provider                   TEXT NOT NULL CHECK (provider IN ('copilot', 'custom', 'azure', 'codex', 'ollama')),
  name                       TEXT NOT NULL,
